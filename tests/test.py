...
def test_convert_number_strings():
    ...
    string_reactions = ["[[0,1],[2,3]]", "[[4,5,6],[7]]"]

    real_reactions = [((0, 1), (2, 3)), ((4, 5, 6), (7,))]

    rg = ReactionGenerator(compounds=compounds)
    converted_string_reactions = list(rg.convert_number_strings(string_reactions))
...
def test_enumerate_combinations():
    ...
    max_two = sorted(list(rg.enumerate_combinations(max_length=2)))
    max_three = sorted(list(rg.enumerate_combinations(max_length=3)))
    max_four = sorted(list(rg.enumerate_combinations(max_length=4)))

    assert max_two == real_max_two
...
def test_iterate():
    ...
    assert three_reaction == [
        "2 CO2 = 2 CO + 1 O2",
        "2 H2O = 2 H2 + 1 O2",
        "2 NO2 = 1 N2 + 2 O2",
    ]

    assert four_reaction == [
