...
class ReactionGenerator:
    ...
    def numeric_reaction_filter(reaction: list) -> str:
        ...
        if not reactants == products:
            if not any([x in reactants for x in products]):
                return str(sorted(tuple((reactants, products))))
    ...
    def enumerate_combinations(self, max_length=4):  # ->tuple:
        """
        enumerates possible combinations of self.compounds as a numeric entity given a max length (minimum reaction length of 2)
        ...
        for reaction_length in range(2, max_length + 1):
            sizing = [
                ...
                for x in it.combinations_with_replacement(
                    np.arange(1, reaction_length), 2
                )
                if np.sum(x) == reaction_length
            ]

            for i, size in enumerate(sizing):
                reactants = it.combinations([x for x in range(self.nc)], size[0])

                products = it.combinations([x for x in range(self.nc)], size[1])
                combined = tuple(it.product(reactants, products))
                filtered = []
    ...
    def find_coefficients(fractional_coefficients: list) -> int:
        ...
        return functools.reduce(lambda a, b: a * b // math.gcd(a, b), denoms)
    ...
    def balance_reaction(self, reactants: dict, products: dict) -> str:
        ...
        reactant_coeff = [symbols(f"r{i}") for i in range(len(reactants))]
        product_coeff = [symbols(f"p{i}") for i in range(len(products))]
        equations = []

        # For each element, write a conservation equation
        for element in elements:
            reactant_sum = sum(
                reactants[species].get(element, 0) * reactant_coeff[i]
                for i, species in enumerate(reactants)
            )
            product_sum = sum(
                products[species].get(element, 0) * product_coeff[i]
    ...
    def balance_function(self, iterable: list):
        ...
        products = {molecule: self.parse_molecule(molecule) for molecule in iterable[1]}
    ...
    def iterate(
        ...
        **tqdm_pathos_kws: dict,
        ...
        numeric_reactions = self.enumerate_combinations(
            max_length=int(max_length)
        )  # this is a generator
        strings = self.convert_to_string(numeric_reactions)  # this is a list...
        # screened = tqdm_pathos.map(self.mp_function,strings)

        screened = tqdm_pathos.map(
            self.balance_function, strings, n_cpus=n_cpus, **tqdm_pathos_kws
        )
        balanced = []
    ...
    def to_pymatgen(self) -> list:
        ...
        try:
            ...
            return [BalancedReaction.from_dict(r) for r in self.as_dict().values()]
        except ImportError:
    ...
    def to_networkx_graph(self):
        ...
        try:
            ...
            for reaction, breakdown in self.as_dict().items():
                ...
                [
                    G.add_node(molecule)
                    for molecule in list(breakdown["reactants"])
                    + list(breakdown["products"])
                ]
                [
                    G.add_edge(reactant, reaction)
                    for reactant in list(breakdown["reactants"])
                ]
                [
                    G.add_edge(reaction, product)
                    for product in list(breakdown["products"])
                ]
            return G
        except ImportError:
